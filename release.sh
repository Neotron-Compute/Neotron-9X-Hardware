...
echo "Running ERC..."
${DOCKER_CMD} python -m kicad-automation.eeschema.schematic run_erc /kicad-project/Kicad/${PROJECT}.sch ${TEMP_PATH_IN_DOCKER}/erc
cp ${TEMP_PATH}/erc/${PROJECT}.erc ${DIR}/build

echo "Printing Schematic..."
${DOCKER_CMD} python -m kicad-automation.eeschema.schematic export -a -f pdf /kicad-project/Kicad/${PROJECT}.sch ${TEMP_PATH_IN_DOCKER}/schematic
cp ${TEMP_PATH}/schematic/${PROJECT}.pdf ${DIR}/build/${PROJECT}-schematic.pdf

echo "Running DRC..."
${DOCKER_CMD} python -m kicad-automation.pcbnew_automation.run_drc /kicad-project/Kicad/${PROJECT}.kicad_pcb ${TEMP_PATH_IN_DOCKER}/drc
cp ${TEMP_PATH}/drc/drc_result.rpt ${DIR}/build/${PROJECT}.drc

echo "Generating Gebers..."
${DOCKER_CMD} python -m kicad-automation.pcbnew_automation.plot /kicad-project/Kicad/${PROJECT}.kicad_pcb ${TEMP_PATH_IN_DOCKER}/gerbers ${LAYERS}
cp ${TEMP_PATH}/gerbers/${PROJECT}_gerbers.zip ${DIR}/build/${PROJECT}-gerbers.zip

echo "Plotting PCB as PDF..."
${DOCKER_CMD} python -m kicad-automation.pcbnew_automation.plot -f pdf /kicad-project/Kicad/${PROJECT}.kicad_pcb ${TEMP_PATH_IN_DOCKER}/pcb ${LAYERS}
